...
// Header for LiquidByteViewArrayV2 serialization
#[repr(C)]
...
impl<B: FsstBuffer> LiquidByteViewArrayV2<B> {
    /*
    Serialized LiquidByteViewArrayV2 Memory Layout:
...
impl LiquidByteViewArrayV2<MemoryBuffer> {
    /// Deserialize a LiquidByteViewArrayV2 from bytes.
    pub fn from_bytes(
        bytes: Bytes,
        compressor: Arc<Compressor>,
    ) -> LiquidByteViewArrayV2<MemoryBuffer> {
        ...
        // TODO: Convert directly to compact offset view 
        let mut offset_views: Vec<CompactOffsetView> = Vec::new();
        if view_header.offset_views_size > 0 {
            ...
            for i in 0..count {
                ...
                offset_views.push(CompactOffsetView::from_parts(off, prefix7, len));
            }
        }
        cursor = offsets_end;
...
#[repr(C)]
pub(crate) struct CompactOffsetView<const N: usize> {
    offset_residual: [u8; N],
    prefix7: [u8; 7],
    len: u8,
}

const _: () = if std::mem::size_of::<CompactOffsetView>() != 12 {
    panic!("CompactOffsetView must be 12 bytes")
};

impl CompactOffsetView {
...
impl LiquidArray for LiquidByteViewArrayV2<MemoryBuffer> {
...
impl LiquidHybridArray for LiquidByteViewArrayV2<DiskBuffer> {
...
impl LiquidByteViewArrayV2<DiskBuffer> {
    ...
    fn compare_equals_with_prefix(&self, needle: &[u8]) -> Option<BooleanArray> {
        ...
        let prefix_len = CompactOffsetView::prefix_len();
...
fn filter_inner<B: FsstBuffer>(
    array: &LiquidByteViewArrayV2<B>,
    filter: &BooleanBuffer,
) -> LiquidByteViewArrayV2<B> {
...
#[derive(Clone)]
pub struct LiquidByteViewArrayV2<B: FsstBuffer> {
    ...
    offset_views: Arc<[CompactOffsetView]>,
...
impl<B: FsstBuffer> std::fmt::Debug for LiquidByteViewArrayV2<B> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LiquidByteViewArrayV2")
...
impl<B: FsstBuffer> LiquidByteViewArrayV2<B> {
    /// Create a LiquidByteViewArrayV2 from an Arrow StringViewArray
    ...
    /// Create a LiquidByteViewArrayV2 from an Arrow BinaryViewArray
    ...
    /// Create a LiquidByteViewArrayV2 from an Arrow StringArray
    ...
    /// Create a LiquidByteViewArrayV2 from an Arrow BinaryArray
    ...
    pub fn train_from_string_view(
        array: &StringViewArray,
    ) -> (Arc<Compressor>, LiquidByteViewArrayV2<MemoryBuffer>) {
...
impl<B: FsstBuffer> LiquidByteViewArrayV2<B> {
    ...
    /// Core implementation that converts a CheckedDictionaryArray to LiquidByteViewArrayV2
    fn from_dict_array_inner(
        ...
        for (i, byte_offset) in byte_offsets.iter().enumerate().take(values.len()) {
            ...
            // Build CompactOffsetView from the suffix bytes directly to avoid leaking encoding details
            ...
            offset_views.push(CompactOffsetView::new(*byte_offset, remaining_bytes));
        }

        assert_eq!(values.len(), byte_offsets.len() - 1);
        offset_views.push(CompactOffsetView::from_parts(
            byte_offsets[values.len()],
            [0u8; 7],
            0,
        ));

        // TODO: Convert to compact offset view 
        LiquidByteViewArrayV2 {
    ...
    fn compare_with_inner(&self, needle: &[u8], op: &Operator) -> Result<BooleanArray, IoRange> {
        ...
        for i in 0..num_unique {
            ...
            let cmp_len = std::cmp::min(CompactOffsetView::prefix_len(), needle_suffix.len());
                ...
                match &decompressed {
                    ...
                    None => {
                        let (values_buffer, offsets_buffer) = raw_buffer
                            .to_uncompressed_v2(&self.compressor.decompressor(), &self.offset_views);
...
mod tests {
    ...
    fn test_dictionary_view_structure() {
        // Test CompactOffsetView structure
        let offset_view = CompactOffsetView::from_parts(1024, [1, 2, 3, 4, 5, 6, 7], 7);
    ...
    fn test_prefix_extraction() {
        ...
        let compressor = LiquidByteViewArrayV2::<MemoryBuffer>::train_compressor(input.iter());
        let liquid_array =
            LiquidByteViewArrayV2::<MemoryBuffer>::from_string_array(&input, compressor);
    ...
    fn test_compare_equals_with_prefix_decidable_and_ambiguous() {
        ...
        let compressor = LiquidByteViewArrayV2::<MemoryBuffer>::train_compressor(input.iter());
        let in_mem = LiquidByteViewArrayV2::<MemoryBuffer>::from_string_array(&input, compressor);
        ...
        let disk_view = hybrid
            .as_any()
            .downcast_ref::<LiquidByteViewArrayV2<DiskBuffer>>()
            .unwrap()
            .clone();
