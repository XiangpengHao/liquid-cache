...
use store::{BudgetAccounting, CacheConfig, CacheEntryID, CacheStore};
...
impl CachedBatch {
    ...
    fn transcode_to_liquid(&self, states: &LiquidCompressorStates) -> Result<CachedBatch, ()> {
        match self {
            Self::ArrowMemory(array) => {
...
impl LiquidCachedColumn {
    ...
    pub(crate) fn insert_arrow_array(
        ...
        self.cache_store.insert(self.entry_id(row_id), array);
        Ok(())
    }
}
...
impl LiquidCache {
    ...
    pub fn new(batch_size: usize, max_cache_bytes: usize, cache_dir: PathBuf) -> Self {
        ...
        LiquidCache {
            ...
            cache_store: Arc::new(CacheStore::new(config, true)),
...
mod tests {
    ...
    fn test_background_transcoding() {
        ...
        loop {
            ...
            match &entry {
                CachedBatch::LiquidMemory(liquid) => {
