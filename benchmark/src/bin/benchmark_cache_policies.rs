...
fn build_mermaid_chart(points: &[(String, u64, f64)], total_unique_size: u64) -> Option<String> {
    ...
    out.push_str("---\n");
    out.push_str("title: Cache Policy Hit Ratio\n");
    out.push_str("---\n");
    out.push_str("xychart-beta\n");
    out.push_str("    title \"Hit Ratio vs Cache Size\"\n");
    out.push_str("    x-axis \"Cache Size (% of working set)\" 0 --> 100\n");
    out.push_str("    y-axis \"Hit Ratio (%)\" 0 --> 100\n");

    for (policy_idx, policy_points) in series_per_policy.into_iter().enumerate() {
        let mut sorted_points = policy_points;
        sorted_points.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap_or(std::cmp::Ordering::Equal));
        let xs: Vec<String> = sorted_points.iter().map(|(x, _)| fmt_float(*x)).collect();
        let ys: Vec<String> = sorted_points.iter().map(|(_, y)| fmt_float(*y)).collect();
        let policy_name = &policies[policy_idx];
        out.push_str(&format!(
            "    series \"{}\" [{}] [{}]\n",
            policy_name,
            xs.join(", "),
