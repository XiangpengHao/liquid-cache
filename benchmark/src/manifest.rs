...
impl BenchmarkManifest {
    ...
    pub fn read_queries(&self) -> Vec<Query> {
        let mut queries = Vec::new();

        for (index, query_spec) in self.queries.iter().enumerate() {
            let sql = if query_spec.ends_with(".sql") {
                std::fs::read_to_string(query_spec).expect("Failed to read query file")
            } else {
                query_spec.clone()
            };

            let final_sql = if let Some(special_handling) = &self.special_query_handling {
                if let Some(special_sql) = special_handling.get(&(index as u32)) {
                    special_sql.clone()
                } else {
                    sql
                }
